...
class WXBot:
    ...
    def run(self):
        ...
        self.status_notify()
        self.get_contact()
        self.update_contacts_to_server()
        print '[INFO] Done update contacts to server .'
        if self.update_keys_to_server():
            print '[INFO] Update Keys to server succeed'
        print '[INFO] Get %d contacts' % len(self.contact_list)
        print '[INFO] Start to process messages .'
        self.proc_msg()

    def update_contacts_to_server(self):
        '''
        Kevin - update contacts to server 
        '''
        serverurl = 'http://yangli221021.oicp.net:8080/helper/index.php/Home/Show/updateContacts'
        
        with open('contact_list.json', 'r') as f:
            dic = json.loads(f.readline())
        print len(dic)
        for user in dic:
            if user['RemarkName'].replace(' ','')=='':
                continue
            contact = {
                'UserName':user['UserName'].replace(' ',''),
                'NickName':user['NickName'].replace(' ',''),
                'RemarkName':user['RemarkName'].replace(' ','')
            }
            print contact
            r = self.session.post(serverurl, data=contact)
            r.encoding = 'utf-8'
        #,user['NickName'].replace(' ',''),user['RemarkName'].replace(' ','')

    def update_keys_to_server(self):
        '''
        Kevin - update all keys to server 
        '''
        serverurl = 'http://yangli221021.oicp.net:8080/helper/index.php/Home/Show/updateMain'
        url = self.base_uri + '/webwxsendmsg?pass_ticket=%s' % self.pass_ticket
        params = {
            'BaseRequest': self.base_request,
            'Msg': {
                ...
                "ToUserName": 'dddst',
                "LocalID": 'mmmsg_id',
                "ClientMsgId": 'mmmsg_id'
            }
        }
        data = json.dumps(params, ensure_ascii=False).encode('utf8')
        r = self.session.post(serverurl, data={'url':url,'premsg':data})
        r.encoding = 'utf-8'
        return r.text == "1"
